...
//        String cwd = System.getProperty("user.dir");
        String blocksPath = this.getClass().getClassLoader().getResource("GeoLiteCity/GeoLiteCity-Blocks.csv").getPath();
        String locationsPath = this.getClass().getClassLoader().getResource("GeoLiteCity/GeoLiteCity-Location.csv").getPath();
        File blocks = new File(blocksPath);
        File locations = new File(locationsPath);
        if (!blocks.exists()){
            System.out.println("********");
    ...
    public void logCommon(
        ...
        while (System.currentTimeMillis() < end){
            ...
            //如果这个常见日志信息列表记录的是上一个小时的日志，那么将这些旧的常见日志消息归档
    ...
    public Jedis redisConnection(String component){
        ...
        if (configConn == null){
            configConn = new Jedis("192.168.11.81");
            configConn.select(0);
            ...
            while (!quit){
                ...
                while (index < conn.zcard("known:")){
                    ...
                    if (remove != 0){
                        ...
                        if (remove == samples.size()){ //如果散列被清空，则从有序集合中删除计数器，但是记得监控
                            conn.watch(hkey);
                            if (conn.hlen(hkey) == 0) { //判断是否为空，是的话则删除
                                Transaction trans = conn.multi();
